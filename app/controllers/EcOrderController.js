...
module.exports = EcOrderController = Controller.extend({
        ...
        /*_.forEach(allOrders, function(order) {
          self.ecOrderProductDao.getAllByOrder(order.id).success( function(orderProducts){
            async.map(orders, function(order, callback){
              ...
              response.render('ecorder/_index', {orders: orders, allOrders: allOrders, id: id, transformed: transformed});
            });
          });
        });*/
        async.map(allOrders, function(order, callback){
          self.ecOrderProductDao.getAllByOrder(order.id).success( function(orderProducts){
              order.products = orderProducts;
              callback(null, order);
          });
        }, function(err, transformed){
          response.render('ecorder/_index', {orders: transformed, id: id});
        });
      //});
    });
  ...
  create: function (request, response) {
    ...
    console.log(order);
    this.ecOrderDao.save(order, function(modelError, order){
      if(modelError){
