 ...
 */
module.exports = EcProductController = Controller.extend({
  ...
  _index: function(request, response) {
    ...
    self.ecProductDao.getAll().success( function(allProducts){
      self.ecProductDao.getAllByPage(offset, limit).success(function (products) {
        response.render('ecproduct/_index', {products: products, allProducts: allProducts, id: id});
      });
    });
  },
  ...
  show: function(request, response) {
    ...
    this.ecProductDao.get(id).success(function (product) {
      if(!product){
        ...
        response.render('ecproduct/show', {'product': product});
      }
    });
  },
  ...
  new: function (request, response) {
    this.ecCategoryDao.getAll().success( function(categories){
      if (categories.length) {
        response.render('ecproduct/new', {categories: categories});
      } else {
        request.flash('danger', 'Vous devez ajouté au moins une catégorie avant d\'ajouter un produit');
        response.redirect('/eccategory/new');
      }
    });
  },
  ...
  create: function (request, response) {
    ...
    product.thumbnail = request.files.thumbnail.name;
    this.ecProductDao.save(product, function(modelError, product){
      if(modelError){
        request.flash('danger', self._parseValidationError(modelError));
        response.redirect('/ecproduct/new');
      }else{
        request.flash('success', 'Produit sauvegardée.');
        response.redirect(self.baseUrl+'/_index');
      }
    });
  },
